...
export const getBlogDataByCategoriesSlug = async (cat_slug, page = 1, per_page = 10) => {
  var blogs = await getAllBlogsData("array");

  blogs = blogs.filter((blog) => {
    var categories = blog.categories.map((cat) => cat.slug);
    if (categories.includes(cat_slug)) return blog;
  });
  ...
  // console.log();
  return {
    data: blogs.slice((page - 1) * per_page, per_page * page),
    pagination: {
      total_pages: Math.ceil(blogs.length) / per_page,
