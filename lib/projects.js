...
export const getProjectsByCategorySlug = async (slug, page = 1, per_page = 10) => {
  var projects = await getAllProjects("array");

  var temp = [];
  projects.forEach((project) => {
    var tempCategories = project.categories.map((cat) => cat.slug);
  ...
  // console.log(temp);
  return {
    data: temp.slice((page - 1) * per_page, per_page * page),
    pagination: {
      total_pages: Math.ceil(temp.length / per_page),
